...
def blend_bias(
    ...
    feedback_count = 0
    if learning_context:
        feedback_count = int((learning_context.get("feedback_summary") or {}).get("count") or 0)
    ...
    memory_weight = min(0.55, 0.18 + feedback_count * 0.04)
    base_weight = 1.0 - memory_weight
    return max(-1.0, min(1.0, base * base_weight + learn * memory_weight * 2.0))
