...
from trading_ai_engine.ml.hf_pack import (
    build_evolution_events_dataset,
    build_market_training_dataset,
    build_streaming_csv_dataset,
    collect_csv_paths,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "This is separate from Trading AI Workstation's SQLite catalog ingest."
        )
    )
    parser.add_argument(
        "--root",
    ...
    parser.add_argument(
        "--evolution-events",
        action="store_true",
        help="Package Trading AI Workstation evolution events as a Hugging Face Dataset instead of CSV rows.",
    )
    parser.add_argument(
        "--market-training",
        action="store_true",
        help="Package downloaded Indian market supervised rows as a Hugging Face Dataset.",
    )
    parser.add_argument(
        "--symbol",
        help="With --evolution-events, export only one symbol.",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=10_000,
        help="With --evolution-events, maximum events to export.",
    )
    args = parser.parse_args()

    if args.evolution_events or args.market_training:
        try:
            ds = (
                build_market_training_dataset()
                if args.market_training
                else build_evolution_events_dataset(symbol=args.symbol, limit=args.limit)
            )
        ...
        if args.preview_rows > 0:
            rows = [ds[i] for i in range(min(args.preview_rows, len(ds)))]
            print(json.dumps({"preview_rows": rows, "rows": len(ds)}, indent=2, default=str))
            return
        if args.push_to_hub:
            ds.push_to_hub(args.push_to_hub, private=args.private)
            print(json.dumps({"pushed_to": args.push_to_hub, "rows": len(ds)}, indent=2))
            return
        print(
            json.dumps(
                {
                    "message": "Evolution events Dataset built.",
                    "rows": len(ds),
                    "hint": "Use --preview-rows 5 or --push-to-hub user/name.",
                },
                indent=2,
            )
        )
    ...
    if args.root is None:
        parser.error("--root is required unless --evolution-events is set")
