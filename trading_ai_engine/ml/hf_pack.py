...
Trading AI Workstation's SQLite ``ml_datasets`` ingest is intentionally a **catalog**: per-file
...
def build_evolution_events_dataset(symbol: str | None = None, *, limit: int = 10_000):
    """
    Build a Hugging Face Dataset from Trading AI Workstation brain evolution events.
    ...
    rows = db.fetch_evolution_events(symbol=symbol, limit=limit)
...
def build_market_training_dataset():
    """Build a Hugging Face Dataset from the downloaded Indian market supervised frame."""
    from datasets import Dataset

    from trading_ai_engine.ml.market_learn import TRAINING_FRAME_PATH

    if not TRAINING_FRAME_PATH.is_file():
        raise FileNotFoundError(f"No training frame at {TRAINING_FRAME_PATH}")
