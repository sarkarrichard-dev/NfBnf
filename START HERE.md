...
5. Press **Enter** (or choose **Start**) — the server starts and **one browser tab** opens at `http://127.0.0.1:8000` with the React dashboard.
...
7. On **Learning from outcomes**, click **Refresh** — you should see ML model stats and **Hugging Face: Active** if `HF_TOKEN` is set.
...
8. Use **Stop server** in the controller when you want port 8000 closed.
...
2. Start **this** app only: `Start Index Options AI.cmd` → **Start**, or `python -m index_ai.server`.
3. Open **`http://127.0.0.1:8000`** only (not port 5173). Hard-refresh with Ctrl+F5 if the page looks stale.
