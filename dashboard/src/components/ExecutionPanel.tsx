...
export function ExecutionPanel({
  ...
  return (
    <section className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
      <h2 className="mb-3 text-sm font-medium text-slate-300">Execution</h2>
      <div className="flex flex-wrap gap-6">
        <div>
          <p className="mb-2 text-xs text-slate-500">Trading mode</p>
        ...
        <div>
          <p className="mb-2 text-xs text-slate-500">Lots per trade</p>
        ...
        <div className="flex flex-wrap gap-4 text-sm">
          <div>
            <span className="block text-xs text-slate-500">Live open</span>
            <strong>{liveSummary?.open ?? 0}</strong>
          </div>
          <div>
            <span className="block text-xs text-slate-500">Live realized</span>
            <strong>{money(liveSummary?.realized_pnl)}</strong>
          </div>
          <div>
            <span className="block text-xs text-slate-500">Live MTM</span>
            <strong>{money(liveSummary?.open_mtm)}</strong>
          </div>
        </div>
      </div>
      {gates.length > 0 ? (
