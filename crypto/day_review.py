...
def open_positions() -> list[dict[str, Any]]:
    """Lane-level open positions right now, across every strategy — the denorm
    records the lane writes at entry (asset / strategy / side / entry_price / …)."""
    return [
        v["position"]
...
def send_day_summary() -> dict[str, Any]:
    """Build + cache today's review and fire the Telegram day summary. Called at
    23:58 IST; anything still open is listed in the message."""
